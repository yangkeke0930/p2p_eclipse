			 ...
			 $(function(){
				 ...
				 if($("#showBindPhoneModal").size() > 0){
					 ...
					 */
					 ...
					 //1、先将输入验证码的表单转换为ajax表单
					 $("#bindPhoneForm").ajaxForm(function(data){
						 if(data.success){
							 window.location.reload();  //重新刷新当前的页面
						 }else{
							 $.messager.popup(data.msg);  //抛出对应的错误信息
						 }
					 });
					...
					<div class="panel-body el-account">
						<div class="el-account-info">
							...
							<div class="pull-left el-head">
								...
								<p>最后登录时间：${(lastLoginTime?string("yyyy-MM-dd HH:mm:ss"))!}
								</p>
							</div>
							<div class="pull-left"
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<form class="form-horizontal" id="bindEmailForm" method="post"
						action="/sendEmail.do">
						<div class="form-group">
							<label for="emailNumber" class="col-sm-2 control-label">邮箱:</label>
							<div class="col-sm-4">
								<input type="text" class="form-control" id="EmailNumber"
									name="EmailNumber" />
							</div>
						</div>
