			...
			source:function(query,process){
				...
				$.ajax({
					...
					success:function(data){
						if(data && data.length){  //这个data就是一个json对象的数组，{id:xx, username:xxxx}
							process(data);  //process就是交给我们获得得数据之后用来调用得方法，这个方法执行了，下拉列表就出来了
						}
					}
				});
			},
			//用来告诉typeadhead怎么显示json对象得内容
			...
			<div class="col-sm-9">
				...
				<div class="row">
					...
					<form id="searchForm" class="form-inline" method="post" action="/vedioAuth.do">
						...
						<div class="form-group">
						    <label>关键字</label>
						    <input class="form-control" style="width: 100px;" type="text" name="keyWord" value="${(qo.keyWord)!''}" />
						</div>
						<div class="form-group">
						    <label>申请时间</label>
			   	...
			   	<div class="form-group">
				    ...
				    <div class="col-sm-6">
				    	<div class="dropdown" id="autocomplate">
					    	<input type="text" class="form-control" name="loginInfoDisplay" id="loginInfoDisplay" autocomplete="off" />
